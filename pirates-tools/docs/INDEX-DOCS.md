...
| `METHODES.md` | **les techniques de travail, NOMMÉES** — construire une table, ajouter des produits, comment le parseur doit se comporter, prouver qu'un contrôle sert. 22 méthodes, chacune avec la panne qui l'a payée et le code qui l'applique. C'est le document qu'on relit avant de recommencer un chantier du même genre pour une autre marque ou un autre fournisseur |
