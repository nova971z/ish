...
| D-71 | « Je ne veux pas que les packs soient écartés » — les packs clickoutil entrent au catalogue | 02/08 | `RENDU` | Décision qui RENVERSE l'écart du 01/08 (posé pour l'argent : la seule réf d'un titre de pack est celle d'un COMPOSANT). Levée par le suivi PAR NOM : le parseur sort les packs `{titre, prix}` (plafond de liste 100 → 400 — 275 packs mesurés sur la page), le traqueur les apparie via `srcNom`, l'importateur les accepte et les range **en Combos** (forcé — sans ça « Scie + 2 batteries » partait dans Scies). Le verrou d'argent reste entier : jamais un prix de pack sur une réf de composant ; nom en doublon → écarté listé (3 mesurés sur 275, 269 uniques). Porte : 2 assertions nouvelles, 1 sabotage rouge. ⛔ **Reste son geste** : un `dryRun=1` (la réponse portera les packs AVEC prix) → j'importe ≈ 269 fiches Combos → `dryRun=0`. |
