...
| D-61 | Adapter le parseur au format de clickoutil | 01/08 | `RENDU` | ⛔ **Dépend d'une donnée que seul son raccourci peut produire** : relancer le traqueur clickoutil en `dryRun=1` (rien d'autre à changer — D-60 est en ligne après déploiement) et coller la réponse. `diagnostic.verdict` + `extraits` diront comment ce site écrit titres, réfs et prix ; le parseur sera étendu sur ce mesuré, porté, saboté — puis `dryRun=0`. Deviner le balisage d'ici serait l'origine O6. |
