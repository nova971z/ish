...
| D-66 | Importer les accessoires sansRef (lames, mèches, batteries…) en Quincaillerie | 02/08 | `RENDU` | ⛔ **Attend une donnée que seul son raccourci produit** : les prix des sansRef n'entrent dans la réponse qu'avec ce déploiement. Un `dryRun=1` après déploiement donnera `sansRef: [{titre, prix}]` ; l'import se fera dessus (réfs récupérées en fiches normales, accessoires nommés avec `srcNom`, noms en doublon exclus et listés, moulages/inserts refusés). |
