...
| **E-312** | la taille des posters déjà en ligne — 1000 px, **correctement mesurée** | fixer la définition des photos que je pose sur les fiches | ⛔ L'user : « pourquoi la photo de la tronçonneuse est floue ? » **La mesure était juste, le raisonnement faux.** Ces posters-là avaient été faits pour les PETITES CARTES du catalogue ; je les ai recopiés pour une image qui occupe TOUT l'écran de la fiche produit. Deux usages, deux tailles. Mesuré sur trois écrans : le héros dessine l'image sur **1674 px d'écran sur son iPad** (834 CSS × 2), 1553 sur un ordinateur, 1178 sur un téléphone — soit **×1,67 d'agrandissement** pour un fichier de 1000 px. Le navigateur invente les pixels manquants, et ça s'appelle du flou. ⚠️ **CE DÉFAUT EST INVISIBLE À TOUT CONTRÔLE DE POIDS — c'est même l'inverse : un poster trop petit est LÉGER**, donc il passe en souriant, et aucune des 30 portes ne le voyait. Remède : `COTE = 2000` dans `poser-visuel.mjs`, quatre posters refaits (facteur 1,67 → **0,84**), et une mesure P8.6 qui compte les posters trop petits — **299 sur 305** le sont encore, chantier D-61. ⚠️ Une valeur recopiée d'un usage voisin se confronte à SON contexte, la lire correctement ne suffit pas |
