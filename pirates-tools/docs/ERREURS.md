...
| **E-115** | « remplace `&sec=1` par `&dryRun=1` » — conseillé pour qu'il obtienne ses baisses de prix | `dryRun` n'écrit pas, mais il **LIT la collection entière** : ~945 documents par instance, jusqu'à 4 instances par balayage, soit **~3 780 lectures** là où le mode à sec en consommait **ZÉRO**. Son quota Firestore a sauté, et son administration avec | l'user : « on vient d'exploser la limite Firebase, tu m'as fait modifier l'URL ». ⚠️ Le mode à sec avait été créé APRÈS un premier quota épuisé, exactement pour ça. Je l'ai contourné pour un confort de mesure — désarmer un filet qu'on a soi-même posé. Remède : le mode à sec CALCULE désormais les prix sur `products.json` lu sur disque, config par `defaults()` et non `load()` ; il rend la moyenne de baisse et les dix plus fortes sans un seul accès. Porte : la base du harnais EXPLOSE au premier contact, donc un résultat prouve qu'aucune lecture n'a eu lieu — et l'assertion d'hier, qui exigeait « aucun prix calculé à sec », a été REMPLACÉE au lieu d'être supprimée : l'invariant n'est plus « pas de bilan » mais « un bilan sans Firestore » |
