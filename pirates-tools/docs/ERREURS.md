...
| **E-223** | `tests/chevauchement.mjs`, v1 | « aucun tap volé » sous QUATRE sabotages | l'application CACHE le dock pendant le défilement (`dock--hidden`) et le fait réapparaître à l'arrêt ; le harnais mesurait juste après le dernier geste → un dock à opacity 0 est filtré → zéro conflit PAR CONSTRUCTION. Corrigé : on attend le RETOUR de l'élément, et son retour est un PRÉALABLE |
