...
function principal(argv) {
  ...
  if (!categorie) {
    console.error('usage : node scripts/archiver-fiches.js --categorie <nom> [--confirmer]');
    return 2;
  }
  ...
  const re = new RegExp(categorie, 'i');
  const tri = trier(liste, (p) => re.test(String((p && p.category) || '')));

  console.log('');
  console.log('═══ ARCHIVER LES FICHES « ' + categorie + ' » ═══');
  console.log('');
  ...
  const dest = path.join(ARCHIVES, 'fiches-' + categorie.toLowerCase() + '-' + horodatage + '.json');
...
module.exports = { trier: trier, visuelsDe: visuelsDe };
