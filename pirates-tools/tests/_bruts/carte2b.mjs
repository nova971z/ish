...
const r = await page.evaluate(() => {
  ...
  // labels à 190° — balayage de tailles de police
  const FS = window.__fs || 5;
  ...
  for(let i=0;i<4;i++){
    ...
    t.setAttribute('style','fill:#fff;stroke:#0b0b12;stroke-width:'+(FS*0.22).toFixed(2)+';paint-order:stroke;font:800 '+FS+'px system-ui,sans-serif');
  ...
  // jeu minimal entre libellés voisins
  let jeuMin=Infinity;
  for(let i=0;i<bbs.length-1;i++){const A=bbs[i],B=bbs[i+1];
    jeuMin=Math.min(jeuMin, Math.max(A.x,B.x)-Math.min(A.x+A.width,B.x+B.width));}
  return {jeuMin:+jeuMin.toFixed(2), largeurMax:+Math.max(...bbs.map(b=>b.width)).toFixed(1), contourFill:csContour.fill, contourStroke:csContour.stroke,
    anneauFill:csAnneau.fill, clip:g.getAttribute('clip-path'),
    labels:lab, collisions:coll};
});

console.log('police '+FS+'px → largeur max '+r.largeurMax+' u, jeu min entre prix '+r.jeuMin+' u  '+(r.jeuMin>=1.5?'✅':'❌')+'  (labels sur terre : '+r.labels.filter(l=>l.surTerre).length+'/4)');
}
if(1){ console.log('');
console.log('\n── ÎLE');
