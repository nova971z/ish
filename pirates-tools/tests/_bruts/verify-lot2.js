...
async function run(){
  ...
  check('catalogue : 35 produits affichés', cat.cards===35, 'cards='+cat.cards);
  check('nouvelle catégorie « Batteries et chargeurs »', cat.chips.includes('Batteries et chargeurs'));

  // Filtre Batteries et chargeurs → 3 packs énergie
  await page.evaluate(()=>{document.querySelectorAll('#catList .cat-chip').forEach(b=>{if(b.dataset.cat==='Batteries et chargeurs')b.click();});}); await page.waitForTimeout(300);
  const nb = await page.evaluate(()=>document.querySelectorAll('#list .product-card').length);
  check('Batteries et chargeurs = 3 packs', nb===3, 'n='+nb);

  // Combos = doit contenir DCK266P2T (1 ancien pack DCF887N combo... total combos = 3 anciens + 1 = ?)
  ...
  check('Combos contient au moins 4 produits (dont DCK266P2T)', combos>=4, 'n='+combos);

  // PDP du pack énergie DCB118Y2 → specs + prix
  await page.evaluate(()=>{ location.hash='#/produit/dewalt-dcb118y2-qw'; }); await page.waitForTimeout(700);
  const pdp = await page.evaluate(()=>{
    ...
    return { title, specsRows: specs?specs.querySelectorAll('tr').length:0,
      has12ah:/12,0 Ah/.test(specs?specs.innerHTML:''), price: price.replace(/\s+/g,' ').slice(0,40),
      imgSrc: img?(img.getAttribute('src')||img.src||''):'' };
  });
  check('PDP DCB118Y2 : titre chargé', /DCB118Y2/.test(pdp.title), pdp.title.slice(0,50));
  check('PDP : tableau specs rempli', pdp.specsRows>=8, 'lignes='+pdp.specsRows);
  check('PDP : spec vérifiée (12,0 Ah)', pdp.has12ah);
  ...
  // DCF887N réaligné : PDP avec 3D model + specs + prix ~ 94-104€ territorial
  await page.evaluate(()=>{ location.hash='#/produit/dewalt-dcf887n'; }); await page.waitForTimeout(700);
  const dcf = await page.evaluate(()=>{
    const specs=document.getElementById('pdpSpecs');
    return { title:(document.getElementById('pdpTitle')||{}).textContent||'',
      specsRows: specs?specs.querySelectorAll('tr').length:0,
      price:(document.getElementById('pdpPrice')||{}).textContent.replace(/\s+/g,' ').slice(0,30) };
  });
  check('DCF887N réaligné : specs enrichies (>=10 lignes)', dcf.specsRows>=10, 'lignes='+dcf.specsRows);
  check('DCF887N : prix réaligné (~94€, plus 129€)', /9[0-9]|10[0-9]/.test(dcf.price) && !/129/.test(dcf.price), dcf.price);
